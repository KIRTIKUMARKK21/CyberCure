...
class IMTFE(nn.Module):
    ...
    def __init__(self, in_channel=3,device=device):
        ...
        self.SRMConv2D.weight.data=torch.load(r'C:\Users\juutk\OneDrive\Documents\hack36\IMTFEv4.pt')['SRMConv2D.weight']
...
def pre_trained_model(weight_path=r'C:\Users\juutk\OneDrive\Documents\hack36\MantraNetv4.pt',device=device):
...
def check_forgery(model,img_path='/kaggle/input/sample-image',device=device):
...
model_v1 = pre_trained_model(weight_path=r'C:\Users\juutk\OneDrive\Documents\hack36\MantraNetv4.pt',device=device)
...
def st_check_forgery(model,img_path='/kaggle/input/sample-image',device=device):
    ...
    # plt.subplot(1,3,2)
    # plt.imshow((final_output[0][0]).cpu().detach(), cmap='gray')
    # st.image((final_output[0][0]).cpu().detach(), cmap='gray')
    # plt.title('Predicted forgery mask')
    
    # plt.subplot(1,3,3)
    # st.pyplot((final_output[0][0].cpu().detach().unsqueeze(2)>0.2)*torch.tensor(original_image))
    # st.image((final_output[0][0].cpu().detach().unsqueeze(2)>0.2)*torch.tensor(original_image))
    # plt.title('Suspicious regions detected')
...
def load_model():
    model = pre_trained_model(weight_path=r'C:\Users\juutk\OneDrive\Documents\hack36\MantraNetv4.pt',device=device)
    return model 
model = load_model()
st.write("""
...
if file is None:
    ...
    # plt.imshow((prediction[0][0].cpu().detach().unsqueeze(2)>0.2)*torch.tensor(image))
    st_check_forgery(model_v1, img_path=file, device=device)
    output_image = Image.open("output.png")
    st.image(output_image, use_column_width=True)
    # plt.title('Suspicious regions detected')
    st.success("Predicted forgery")